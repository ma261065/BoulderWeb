class Boulder extends Entity {
    constructor(x, y) {
        ...
        this.fallStarted = false; // Track if this boulder has started falling
    }
    ...
    update(grid) {
        ...
        if (Entity.isInBounds(this.x, this.y + 1)) {
            ...
            if (belowType === ENTITY_TYPES.EMPTY) {
                ...
                // This prevents "instant death" when player moves under stationary boulder
            ...
            if (canFall) {
                ...
                this.falling = true;
                this.fallStarted = true; // Mark that this boulder has started falling
                moved = true;
            }
        }
        ...
        // IMPORTANT: Reset fallStarted if boulder is now resting on solid support
        if (!moved && Entity.isInBounds(this.x, this.y + 1)) {
            const belowType = grid[this.y + 1][this.x];
            if (belowType === ENTITY_TYPES.WALL || 
                ...
                // Boulder is supported by something solid - reset fallStarted
    ...
    getLandingSound() {
        return this.justLanded ? 'boulder' : null;
    }
}